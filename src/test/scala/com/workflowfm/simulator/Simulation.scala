  ...
  "Simulations" must {
      ...
      class DummySim(name: String, coordinator: ActorRef)(
        ...
        override def run(): Future[Any] = {
          ...
          val task1 = futureTask(
            id1,
            TaskGenerator("task1", "sim", ConstantGenerator(2L), ConstantGenerator(0L)),
            Seq("r1")
          )
          ready()
          val task2 = task1 flatMap { _ =>
            val t = futureTask(
              id2,
              TaskGenerator("task2", "sim", ConstantGenerator(2L), ConstantGenerator(0L)),
              Seq("r1")
            )
            ack(Seq(id1))
            t
          ...
          val task3 = task2 flatMap { _ =>
            val t = futureTask(
              id3,
              TaskGenerator("task3", "sim", ConstantGenerator(2L), ConstantGenerator(0L)),
              Seq("r1")
            )
            ack(Seq(id2))
            t
    ...
    "innitiate child simulations" in {
      class DummySim(name: String, coordinator: ActorRef)(
        ...
        override def run(): Future[Any] = {
          val childSim = system.actorOf(
            SingleTaskSimulation.props("ChildSim", self, Seq("r1"), ConstantGenerator(2L))
          )
          sim(childSim)
          ready()
        ...
        override def completeActor(actor: ActorRef, time: Long, id: UUID): Unit = promise.success(Unit)
      }
      ...
      sim ! Simulation.SimCompleted(actor, 2L, java.util.UUID.randomUUID)
